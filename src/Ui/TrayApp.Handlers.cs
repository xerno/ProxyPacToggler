...
namespace ProxyPacToggler.Ui
{
    internal sealed partial class TrayApp
    {
        ...
        // Asked after the first sync: a modal dialog must not delay settling the state.
        private void AskForPacUrlIfUnset()
        {
            Guarded("asking for the PAC URL", delegate
            {
                if (controller.Settings.PacUrl.Trim().Length > 0) return;

                if (PromptForPacUrl("")) Synchronise(true, "url");
                else log.Write("no PAC URL yet; the menu can set one later");
            });
        }
